...
#   (G) 2D 粗网格搜（以“探测器质心”为中心，半径=探测环半径+样品半径）
#   (A) Gauss–Newton 微调（3D，对 V 做少量步数最小二乘）
# 运行：python pals_basic_v4.py
...
GRID_MARGIN_MM   = 30.0   # 冗余边界（mm），半径=R_DET_MM + R_SRC_MM + 这个余量
...
# (G) 2D 粗网格搜（以“探测器质心”为中心）
def reconstruct_vertex_grid(R, E, step_mm=GRID_STEP_MM, margin_mm=GRID_MARGIN_MM):
    ex, ey, n = plane_basis(R[0], R[1], R[2])

    # 质心（几何中心）与其在事件平面2D坐标
    C3 = R.mean(axis=0)
    C2 = to_2d(C3, R[0], ex, ey)

    # 覆盖半径：探测环半径 + 样品半径 + margin
    rad = R_DET_MM + R_SRC_MM + margin_mm

    # 以 “C2 为中心” 扫网格
    xs = np.arange(C2[0] - rad, C2[0] + rad + 1e-6, step_mm)
    ys = np.arange(C2[1] - rad, C2[1] + rad + 1e-6, step_mm)
...
def refine_vertex_gauss_newton(V0, R, E, steps=10, lam=1e-6):
...
# 综合：对给定映射下的 E，先闭式，再“质心网格”，取更好 → 微调
def reconstruct_vertex_one_mapping(R, E):
    Vc, ec = reconstruct_vertex_closed(R, E)
    Vg, eg = reconstruct_vertex_grid(R, E, step_mm=GRID_STEP_MM, margin_mm=GRID_MARGIN_MM)
    # 选更好的初值
    if (Vc is not None) and (ec <= eg):
        V0, e0 = Vc, ec
    else:
        V0, e0 = Vg, eg
    # 微调
    Vh, eh = refine_vertex_gauss_newton(V0, R, E, steps=10, lam=1e-6)
...
def main():
    ...
    for V in V_true_all:
        ...
        # 保险：再做一次微调（一般已经很小了）
        V_hat, eps = refine_vertex_gauss_newton(V_hat0, R, Em, steps=6, lam=1e-6)
        if eps > eps0:
