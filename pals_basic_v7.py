# pals_basic_v7.py
# 3γ triangulation – robust "angle-only LSQ" + small polish (numpy only)
# 核心变化：
#   1) 不用任何“闭式三角公式”；改为直接最小化 3 对角度余弦的残差（2D 问题）
#   2) 多起点（以几何质心为中心的小网格）+ 6 种 E↔R 映射；择优
#   3) 可选的 3D 轻量抛光（把闭合向量也纳入；默认步数很小或可关）
#
# 文件单独可跑；只依赖 numpy
...
# ===== 参数（你只需改这里） =====================================================
R_DET_MM     = 250.0     # 探测器半径（等边三点）
R_SRC_MM     = 150.0     # 顶点采样半径（生成器用）
N_EVENTS     = 1000
SEED         = 42        # 固定=可复现；None=每次不同
SAVE_CSV     = True
PRINT_DIAG   = True      # 只对前 3 条事件打印自检
POLISH_STEPS = 6         # 3D 抛光步数（0=不抛光；一般 0~6 足够）
W_ANGLE      = 1.0       # 角度残差权重
W_CLOSURE    = 0.2       # 抛光时闭合项的相对权重（太大会跑向伪谷）

# 2D 最小二乘的多起点网格配置（相对于“几何质心”）
GRID_STEP_MM = 30.0
GRID_RADIUS_MM = 180.0   # 覆盖整块样品 + 一点冗余
...
def solve_energies_from_VR(V, R):
    ...
    if cond > 1e10:
        raise ValueError("ill-conditioned generator A, cond=%.2e"%cond)
    E = np.linalg.solve(A, b)
    if np.any(E <= 0.0):  # 负能量 or 0 视作非物理
        raise ValueError("non-physical E")
    return E
...
def solve_V2_from_angles(r1, r2, r3, cos_hat, grid_step=GRID_STEP_MM, grid_rad=GRID_RADIUS_MM):
    # 多起点：以三点质心为中心，半径 grid_rad，步长 grid_step
    C2 = (r1 + r2 + r3) / 3.0
    ...
    for x in xs:
        for y in ys:
            ...
            # 2D Gauss–Newton（2未知/3残差）
            V = V2.copy()
            for _ in range(12):  # 步数不需要多
...
# ===== 抛光：3D（角度 + 小权重闭合），可设 POLISH_STEPS=0 关闭 ===============
...
def reconstruct_one_mapping(R, E):
    ...
    (_, _cos_hat) = energies_to_thetas_and_cos(E)

    # 2D 角度方程最小二乘（多起点）
    V2, cost2 = solve_V2_from_angles(r1, r2, r3, _cos_hat,
                                     grid_step=GRID_STEP_MM, grid_rad=GRID_RADIUS_MM)
    V3 = from_2d(V2, R[0], ex, ey)

    # 可选：3D 抛光（角度 + 小权闭合）
    V3 = polish_3d(V3, R, E, _cos_hat, steps=POLISH_STEPS)

    # 最终评分：角度主、闭合辅
    r_ang = residual_angle_2d(to_2d(V3, R[0], ex, ey), r1, r2, r3, _cos_hat)
    S     = closure_vec(V3, R, E)
    score = W_ANGLE*np.dot(r_ang, r_ang) + W_CLOSURE*np.dot(S, S)
    return V3, float(score)

def reconstruct_try6(R, E):
...
def main():
    ...
    for idx, V in enumerate(V_true_all):
        ...
        V_hat, sc, perm = reconstruct_try6(R, E)
        if V_hat is None: 
            continue
